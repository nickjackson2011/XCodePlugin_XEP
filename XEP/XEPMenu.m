...
- (NSMenu *)createXEPMenu
{
	...
	[xep addItem:item];
	[item release];
    item = [[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:@".m File Top Function Comment" action:@selector(createMFileTopComment:) keyEquivalent:@"T"];
	[item setKeyEquivalentModifierMask:(NSShiftKeyMask | NSControlKeyMask)];
	item.target = self;
	[xep addItem:item];
	[item release];
    item = [[NSMenuItem allocWithZone:[NSMenu menuZone]] initWithTitle:@".m File Normal Function Comment" action:@selector(createMFileNormalComment:) keyEquivalent:@"M"];
	[item setKeyEquivalentModifierMask:(NSShiftKeyMask | NSControlKeyMask)];
	item.target = self;
	[xep addItem:item];
	[item release];
...
#pragma mark - Create m file comment
- (void)createMFileNormalComment:(id)sender
{
	@synchronized(self) {
		@try {
			NSWindow	*nsKeyWindow	= [[NSApplication sharedApplication] keyWindow];
            ...
            [ocomment appendString:@"#pragma mark <#Description#>\n"];
            int insertIdx = (int)[lua tointeger:-1];
            rg			= [ls rangeOfLineNumber:(sline + insertIdx)];
            rg.length	= 0;
            [tv insertText:ocomment replacementRange:rg];
...
- (void)createMFileTopComment:(id)sender
{
	@synchronized(self) {
		@try {
			NSWindow	*nsKeyWindow	= [[NSApplication sharedApplication] keyWindow];
